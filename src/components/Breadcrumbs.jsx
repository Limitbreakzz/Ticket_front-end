...
const FALLBACK_REGISTRY = {
  ...
  'create-ticket': { section: 'การดำเนินการ', label: 'แจ้ง Ticket ใหม่', icon: 'ticket' },
  ...
  sla: { section: 'วิเคราะห์', label: 'ติดตามกำหนดเวลา (SLA)', icon: 'clock' },
...
export default function Breadcrumbs() {
  const { role, activeNav, setActiveNav, activeTicketId } = useApp();
  ...
  if (activeTicketId) {
    trail.push({ label: `รายละเอียด #${activeTicketId.substring(0, 8)}`, icon: 'ticket' });
  }
